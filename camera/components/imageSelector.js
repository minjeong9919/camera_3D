...
export const imageSelector = () => {
  ...
  const containerGeometry = new THREE.ExtrudeGeometry(
    containerShape,
    extrudeSettings
  );
  const container = new THREE.Mesh(containerGeometry, plasticMaterial);
  ...
  const up = new THREE.Mesh(geometry, plasticMaterial);
  up.position.y = -0.1;
  up.position.x = -1;
  up.position.z = -0.56;
  up.rotateZ(Math.PI);
  ...
  const enter = new THREE.Mesh(enterGeometry, plasticMaterial);
